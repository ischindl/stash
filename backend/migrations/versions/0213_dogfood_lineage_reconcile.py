...
`0204` — but that string refers to *different content* than main's `0211`, so
`alembic upgrade head` skips main's `0211` as "already applied": the widened
...
- founder DB (dogfood-stamped): the CHECK is rebuilt with the widened set, which
  is the content main's `0211` was supposed to apply; `models_json_enc` already
...
- fresh/main DB: gains `models_json_enc` (main's routers and `agent_auth` store
  ...
  a no-op against the set main's `0211` already applied.
